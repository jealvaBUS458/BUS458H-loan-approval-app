...
using a Logistic Regression model built for the BUS458 final project.
"""
)
...
Adjust the inputs to see how the predicted approval probability changes.
"""
    )
    st.markdown("---")
...
if st.button("Predict Approval Probability"):
    ...
    if proba >= threshold:
        st.write(
            ...
            "given the current model and cutoff. They may be a good candidate "
            "for routing to this lender."
        )
    else:
        st.write(
            "This applicant has a lower predicted chance of approval under the "
            "current model and cutoff. The platform may want to decline or consider "
            "alternative lenders or terms."
        )
else:
